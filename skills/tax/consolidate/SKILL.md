...
resources/ (informes) ──[/read]──► processed/informes.json ──[/generate]──► irpf_consolidated.xlsx
                                                  ▲
                       processed/brazil_investments.xlsx (b3) ─────────────┘
```
...
2. **[/generate](../generate/SKILL.md)** — script determinístico que junta
   `processed/brazil_investments.xlsx` + `processed/informes.json` no `irpf_consolidated.xlsx`.
...
- `processed/brazil_investments.xlsx` (saída do [b3](../b3/SKILL.md)).
...
2. **/generate** — monte as fichas (saída na raiz):
   `python ../generate/scripts/generate.py --investimentos processed/brazil_investments.xlsx --json processed/informes.json --outdir .`
3. **Leia o AUDIT** do generate: ativo B3 na `brazil_investments.xlsx` ausente do `informes.json` é
...
- **Fontes da verdade:** valor/custo de ativo B3 → `brazil_investments.xlsx`; classificação
