...
|---|---|---|
| Bens e Direitos **valor** (B3 asset) | `brazil_investments.xlsx` (b3) | acquisition cost = preço médio × quantidade |
...
The script reads **only** the `IRPF_bens_e_direitos` sheet of `brazil_investments.xlsx`: per ticker it
...
### Field notes
- **`b3`** (bens): `true` → value is pulled from `brazil_investments.xlsx` by `key` (match is
