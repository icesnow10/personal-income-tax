---
name: irpf
description: Orchestrate the full Brazilian IRPF investments pipeline end to end — b3 → consolidate → completeness. Runs b3 to reconstruct preço médio / custo from the B3 exports (b3_brazil_renda_variavel_avg_price_calculation.xlsx), consolidate (which itself runs /read to transcribe every informe in resources/ into processed/informes.json, then /generate to build the final IRPF fichas irpf_consolidated.xlsx), then completeness to audit the result against the informes (completeness_report.md). Use when the user wants to "fazer o IRPF dos investimentos", run the whole pipeline, montar a declaração de investimentos do zero, or mentions /irpf.
---
...
resources/ (B3 exports + informes)
        │
        ▼
   [1] b3 ──────────────►  processed/b3_brazil_renda_variavel_avg_price_calculation.xlsx   (preço médio / custo — fonte da verdade do valor)
        │
        ▼
...
| `processed/` | **derivado**: `b3_brazil_renda_variavel_avg_price_calculation.xlsx` + a transcrição `informes.json` |
...
### 1. b3 → `processed/b3_brazil_renda_variavel_avg_price_calculation.xlsx`
...
- **/generate** — monte as fichas (saída na raiz):
  ```
  python ../generate/scripts/generate.py --investimentos processed/b3_brazil_renda_variavel_avg_price_calculation.xlsx \
      --json processed/informes.json --outdir .
  ```
  Gera **um** arquivo com as abas `bens_e_direitos`, `isentos` e `exclusiva_definitiva`.
  **Leia o AUDIT**: ativo B3 na `b3_brazil_renda_variavel_avg_price_calculation.xlsx` ausente do `informes.json` é listado —
...
`python ../completeness/scripts/fetch_escriturador.py --investimentos processed/b3_brazil_renda_variavel_avg_price_calculation.xlsx`.
...
|---|---|---|
| `processed/b3_brazil_renda_variavel_avg_price_calculation.xlsx` | b3 | preço médio/custo por ticker + abas IRPF auxiliares |
