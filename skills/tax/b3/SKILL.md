...
Two markdown files drive the run and are pasted into the workbook's `aux_mapping` sheet. Edit
...
Keep your filled `ticker_memory.md` in your **working folder** and point `--memory-dir` at it.
`mapping_memory.md` falls back to the bundled one if not present.
