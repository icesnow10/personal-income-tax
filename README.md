...
- **B3 exports** *(only if you have/had assets custodiados na B3 — ações, FIIs, BDRs, Tesouro, renda fixa)* —
  ...
  **Movimentação** (the full history since your first trade — what `variable_income_brazil` replays to
  ...
  Foreign stocks/ETFs (Avenue, Nomad, Interactive Brokers) aren't on the B3 — `variable_income_international`
...
resources/  (broker/bank informes + B3 "Movimentação"/"Posição" exports)
 ...
 [2] variable_income  (renda variável — Brasil + exterior):
     ├─ variable_income_brazil ─────►  …variable_income_avg_price….xlsx  preço médio of ações/FII/BDR on the B3
     └─ variable_income_international►  variable_income_international.xlsx ações/ETF abroad (from the informe — not on the B3)
     │
...
Sources of truth, per ficha: **Brazilian renda variável value** → `variable_income_brazil` (preço médio);
**foreign equity value** → `variable_income_international` (the broker informe — no B3 to validate against);
...
│  ├─ ticker_memory.md             renames / incorporações (variable_income_brazil)
│  ├─ mapping_memory.md            B3 movement → action (variable_income_brazil)
...
│  ├─ b3_brazil_variable_income_avg_price_calculation.xlsx  preço médio of ações/FII/BDR (variable_income_brazil)
│  ├─ variable_income_international.xlsx  ações/ETF abroad, value from the informe (variable_income_international)
...
|---|---|---|
| [irpf](skills/tax/irpf/SKILL.md) | `read` · `variable_income` · `fixed_income` · `consolidate` · `completeness` | Orchestrator — runs the full investments pipeline end to end (5 steps) from the broker/bank informes + B3 exports. |
...
| [variable_income](skills/tax/variable_income/SKILL.md) | `variable_income_brazil` · `variable_income_international` | Orchestrates the whole renda-variável slice — Brazilian (B3) and foreign — and feeds both into `consolidate`. |
| └ [variable_income_brazil](skills/tax/variable_income_brazil/SKILL.md) | — | Reconstructs the average acquisition cost (**preço médio**) of B3 renda variável (ações, FIIs, BDRs) from the **Movimentação** history + year-end **Posição**. Driven by editable **living-memory files** (movement→action mapping, ticker renames). On-B3 renda variável only. |
| └ [variable_income_international](skills/tax/variable_income_international/SKILL.md) | — | Builds **foreign** stocks/ETFs (Avenue, Nomad, Interactive Brokers, …) from `informes.json` — these aren't on the B3, so there's no preço médio: the value comes straight from the broker informe. Flags foreign income for the correct ficha (carnê-leão/GCAP). |
...
| [consolidate](skills/tax/consolidate/SKILL.md) | — | Merges the four sources — variable_income_brazil workbook + variable_income_international workbook + fixed_income workbook + `informes.json` (everything else) — into `irpf_consolidated.xlsx`, the 3 IRPF fichas. Slices owned by the workbooks aren't re-read (no double counting). |
